	...
	return (
		<Card className="cursor-pointer group overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-linear-to-br from-white to-gray-50">
			<div className="relative h-64">
				...
				{/* Universe badge */}
				<Badge
					className={`absolute top-3 right-3 text-xs ${hero.universe === "DC" ? "bg-blue-600" : "bg-red-600"}`}
				>
					{hero.universe}
				</Badge>
			...
			<CardHeader className="py-3 z-10 bg-gray-100/50 backdrop-blur-sm relative top-1 group-hover:-top-2.5 transition-all duration-300">
				...
				<div className="flex flex-wrap gap-2">
					{teams.slice(0, 2).map((team) => (
						<Badge key={team} variant="outline" className="w-fit text-xs">
							{team}
						</Badge>
					))}

					{teams.length > 2 && (
						<Badge variant="outline">+{teams.length - 2} más</Badge>
					)}
				</div>
			</CardHeader>
